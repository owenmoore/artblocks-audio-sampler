...
const PROJECT_EXPLORER = 'https://api.thegraph.com/subgraphs/name/artblocks/art-blocks'

const contracts = ['0x059edd72cd353df5106d2b9cc5ab83a52287ac3a', '0xa7d8d9ef8d8ce8992df33d8b8cf4aebabd5bd270']
...
async function fetchPlatform() {
  ...
  contracts {
    ...
    const nextProjectId = contracts.reduce((max, c) => Math.max(max, parseInt(c.nextProjectId, 10)), 0)
    return {
...
`,
    ...
    if (tokens.length > 1) throw new Error('Received multiple tokens for id: ' + id)
    const t = tokens[0]
    return {
        id: t.tokenId,
        // owner: t.owner.id,
...
export {fetchProject, fetchPlatform, fetchTokensByProject, fetchTokensByOwner, fetchToken, query}
